...
class ArticleDetailViewController: UIViewController {
	...
	private var webViewArray: [ArticleDetailView_T] = [
		ArticleDetailView_T(),
		ArticleDetailView_T(),
		ArticleDetailView_T()
	]
	...
	private func configureWebView(at index: Int) {
		...
		webView.snp.makeConstraints { make in
			make.top.equalToSuperview()
			make.leading.equalToSuperview().offset(Constants.width * CGFloat(index) + 10)
			make.width.equalTo(Constants.width - 20)
			...
			if scrollView == webViewArray[1].scrollView {
				webViewArray[1].adjustImageView {
					$0.snp.updateConstraints { make in
						make.height.equalTo(400 - scrollView.contentOffset.y - 210)
//						make.bottom.equalTo(scrollView.snp.top).offset(210 + scrollView.contentOffset.y*0.5)
					}
				}
			}
		}
...
//			print("changed")
	...
	func moveToRight() {
		webViewArray[2].snp.updateConstraints { make in
			make.leading.equalToSuperview().offset(Constants.width + 10)
		}
		setSwitchingViewContentOffset()
		...
		webViewArray[0].snp.updateConstraints { make in
			make.leading.equalToSuperview().offset(Constants.width * CGFloat(2) + 10)
		}
		view.layoutIfNeeded()
